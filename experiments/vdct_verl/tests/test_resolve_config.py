...
import pytest
from vdct_test_helpers import load_script

resolve_config = load_script("resolve_config") if importlib.util.find_spec("hydra") else None


def _verl_checkout() -> Path | None:
    candidate = os.environ.get("VERL_DIR", "/home/user/volcengine/verl")
    path = Path(candidate)
    return path if (path / "verl" / "trainer" / "config" / "ppo_trainer.yaml").exists() else None


pytestmark = pytest.mark.skipif(
    resolve_config is None or _verl_checkout() is None,
    reason="needs hydra-core and a verl checkout (VERL_DIR)",
)


def test_resolves_and_prints_the_run_plan(capsys):
    assert resolve_config.main(["--verl-dir", str(_verl_checkout()), "data.train_batch_size=64"]) == 0
...
def test_rejects_v1_trainer(capsys):
    assert resolve_config.main(["--verl-dir", str(_verl_checkout()), "trainer.use_v1=True"]) == 1
    assert "use_v1" in capsys.readouterr().err


def test_rejects_reenabled_verl_kl(capsys):
    assert resolve_config.main(["--verl-dir", str(_verl_checkout()), "actor_rollout_ref.actor.use_kl_loss=True"]) == 1
    assert "KL" in capsys.readouterr().err


def test_rejects_misaligned_batch_size(capsys):
    assert resolve_config.main(["--verl-dir", str(_verl_checkout()), "data.train_batch_size=66"]) == 1
